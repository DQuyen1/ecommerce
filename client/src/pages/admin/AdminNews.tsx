import { useState } from "react";
import { Link } from "react-router-dom";
import { api } from "../../api";
import { useAsync } from "../../hooks/useAsync";
import { Empty, ErrorState, Loading } from "../../components/States";
import { IconPencil, IconPlus, IconTrash } from "../../components/Icon";
import { useAdminRedirectOnAuthError } from "../../hooks/useAdminGuard";

export default function AdminNews() {
  const [refreshKey, setRefreshKey] = useState(0);
  const [actionError, setActionError] = useState<string | null>(null);
  const { data, loading, error } = useAsync(
    () => api.news.list({ limit: 200 }),
    [refreshKey]
  );
  useAdminRedirectOnAuthError(actionError);

  async function handleDelete(id: string, title: string) {
    if (!window.confirm(`Xóa bài viết "${title}"? Không thể hoàn tác.`)) return;
    setActionError(null);
    try {
      await api.admin.news.remove(id);
      setRefreshKey((k) => k + 1);
    } catch (err) {
      setActionError((err as Error).message);
    }
  }

  return (
    <>
      <div className="admin-page-head">
        <div>
          <h1>Tin Tức</h1>
          <p>Bài viết hiển thị công khai trên trang web.</p>
        </div>
        <Link className="btn btn-sm" to="/admin/news/new">
          <IconPlus />
          Thêm Bài Viết
        </Link>
      </div>

      {actionError && (
        <div className="alert alert-error" style={{ marginBottom: 18 }}>
          {actionError}
        </div>
      )}

      {loading && <Loading />}
      {error && <ErrorState message={error} />}
      {data && data.items.length === 0 && <Empty label="Chưa có bài viết nào." />}

      {data && data.items.length > 0 && (
        <div className="admin-table-wrap">
          <table className="admin-table">
            <thead>
              <tr>
                <th>Bài viết</th>
                <th>Chủ đề</th>
                <th>Ngày đăng</th>
                <th></th>
              </tr>
            </thead>
            <tbody>
              {data.items.map((article) => (
                <tr key={article.id}>
                  <td className="admin-col-title">{article.title}</td>
                  <td className="admin-col-meta">{article.topic}</td>
                  <td className="admin-col-meta">
                    {new Date(article.publishedAt).toLocaleDateString("vi-VN")}
                  </td>
                  <td>
                    <div className="admin-actions">
                      <Link
                        className="admin-icon-btn"
                        to={`/admin/news/${article.id}/edit`}
                        aria-label="Sửa"
                      >
                        <IconPencil />
                      </Link>
                      <button
                        type="button"
                        className="admin-icon-btn is-danger"
                        aria-label="Xóa"
                        onClick={() => handleDelete(article.id, article.title)}
                      >
                        <IconTrash />
                      </button>
                    </div>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      )}
    </>
  );
}
